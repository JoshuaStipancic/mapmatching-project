...
# 
#   # Create the directory (if necessary) and save the output file
#   dir.create(filepath, recursive=TRUE)
#   write.table(results, file = file.path(filepath, str_replace(output, "xlsx", "csv")), sep=",",  col.names=FALSE)
# 
#   # Format and ouput run time results
#   write(Time_T,file.path(filepath, str_replace(output, ".xlsx", "_time.txt")))
# }
...
# for(filename in Sys.glob("Data/Scenario 3/5*/3AutoMerit/*.xlsx"))
# {
# 
#   # Read one file
#   trip <- read_xlsx(filename)
...
for (filename in Sys.glob("Test/*/*/1GPS-Tracker_APP/*.csv"))
{
